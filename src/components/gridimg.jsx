...
const ImageCard = ({ imagePath, text }) => {
  ...
  return (
    <div className="relative" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
      <img src={imagePath} alt={text} className="w-[169px] h-[115px] object-cover" />

      {/* <div
        className={`absolute inset-0 flex items-center justify-center  bg-[#274C6999] transition duration-300 ease-in-out ${
          isHovered ? 'bg-[#E4C1C999] text-bblue' : 'text-white'
        } font-extrabold`}
      >
        <span className="  max-w-[70px] h-[48px] leading:[21px] text-[16px] md:max-w-[180px] md:h-[74px] md:text-[40px] font-extrabold font-montserrat md:leading-[50px] text-center">
          {text}
        </span>{' '}
...
const ImageGrid = ({ images }) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto grid grid-cols-2 p-2 gap-4 md:grid-cols-5 align-middle py-15 justify-center ">
      {visibleImages.map((image, index) => (
        <ImageCard key={index} imagePath={image.path} text={image.text} />
      ))}

      {/* Return Button */}
      {/* <div className="flex justify-center w-full col-span-2 mt-4 mb-8 md:justify-end md:col-span-3">
